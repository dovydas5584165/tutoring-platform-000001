...
import { format, parseISO, addDays, addWeeks, isSameDay } from "date-fns";
...
export default function ScheduleLanding() {
  ...
  const slug = decodeURIComponent(params.slug as string); // subject
  ...
  const nextMonthDates = Array.from({ length: 35 }, (_, i) =>
    addDays(new Date(), i)
  );
  ...
  return (
    <div className="flex flex-col min-h-screen bg-white text-gray-900 font-sans">
      ...
      <main className="flex flex-col flex-1 items-center px-8 py-12 w-full max-w-4xl mx-auto">
        ...
        {/* Kalendorius su datomis */}
        {selectedTeacher && (
          <div className="w-full mb-8">
            <p className="mb-2 text-sm text-gray-700 font-medium">Pasirinkite dieną:</p>
            <div className="grid grid-cols-3 sm:grid-cols-5 md:grid-cols-7 gap-2 mb-6">
              {nextMonthDates.map((date) => {
                const formatted = format(date, "EEE, MMM d", { locale: lt });
            ...
            {loading ? (
              ...
              <p className="text-gray-600">Nėra laisvų laikų šiai dienai.</p>
            ) : (
