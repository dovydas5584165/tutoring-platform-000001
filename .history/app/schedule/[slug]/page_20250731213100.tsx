...
export default function ScheduleLanding() {
  ...
  const daysToShow = Array.from({ length: 7 }, (_, i) => addDays(startOfToday(), i));
  ...
  const filteredSlots = slots.filter((slot) =>
    isSameDay(new Date(slot.start_time), selectedDay)
  );
  ...
  return (
    <div className="flex flex-col min-h-screen bg-white text-gray-900 font-sans">
      ...
      <main className="flex flex-col flex-1 items-center px-6 py-10 w-full max-w-4xl mx-auto">
        ...
        {/* Day selector */}
        {selectedTeacher && (
          <div className="w-full mb-6 flex gap-2 overflow-x-auto">
            {daysToShow.map((day) => (
              <button
        ...
        {selectedTeacher && (
          <div className="w-full">
            {loading ? (
              ...
              <p className="text-gray-600">Šiai dienai nėra galimų laikų.</p>
            ) : (
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                {filteredSlots.map((slot) => {
                  ...
                  return (
                    <div
                      key={slot.id}
                      className={`relative p-4 border rounded-lg shadow-sm cursor-pointer transition-all duration-200 text-center ${
                        isSelected
