...
export default function StudentDashboard() {
  ...
  useEffect(() => {
    (async () => {
      ...
      /* If only one point, clone it slightly to draw a line */
      if (m.length === 1) {
        m.push(m[0] + 0.2); // tiny offset on x‑axis
        g.push(g[0]);
      }
  ...
  const chartOptions: ChartOptions<"line"> = {
    ...
    scales: {
      y: { min: 0, max: 10, ticks: { stepSize: 1 }, title: { display: true, text: "Pažymys" } },
      x: { title: { display: true, text: "Mėnuo" } },
    },
  };

  /* ----------------------------------------------------------------- */
  /* Static demo sections (lessons, assignments)                       */
  /* ----------------------------------------------------------------- */
