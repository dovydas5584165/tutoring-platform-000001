...
function NotificationBanner({
  ...
  return (
    <div
      className={`fixed top-4 left-1/2 transform -translate-x-1/2 px-6 py-3 rounded shadow-lg text-white z-50 ${bgColor} flex items-center justify-between max-w-lg w-full`}
    >
...
export default function TutorDashboard() {
  ...
  useEffect(() => {
    const fetchAvailability = async () => {
      ...
      if (error) {
        ...
        setNotification({ type: "error", message: "Klaida gaunant prieinamumą." });
        return;
      }
  ...
  const handleSave = async () => {
    ...
    if (error) {
      ...
      setNotification({ type: "error", message: "Klaida atnaujinant prieinamumą." });
      return;
    }
  ...
  const acceptTerms = async () => {
    try {
      ...
      setShowTerms(false);
      setNotification({ type: "success", message: "Sutartis priimta." });
    } catch (err) {
  ...
  const handleLogout = async () => {
    try {
      ...
      if (!error) {
        router.push("/auth/log-in");
        setNotification({ type: "success", message: "Sėkmingai atsijungta." });
      } else {
