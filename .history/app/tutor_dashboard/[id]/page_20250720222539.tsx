...
// Terms & Conditions popup component
...
export default function TutorDashboard() {
  ...
  // Check if user agreed to terms on component mount
  useEffect(() => {
  ...
  // User accepts terms — update DB and UI state
  ...
  // Sample lessons (replace with real data or DB call)
  ...
  // Navigation helper for tutor subpages
  const goTo = (path: string) => {
    if (id) router.push(`/tutor_dashboard/${id}/${path}`);
  };

  // Show Terms Popup if user hasn't accepted terms yet
  if (showTerms) {
          ...
          <ul className="space-y-3">
            {lessons.map((lesson) => (
              <li key={lesson.id} className="border-b border-gray-200 pb-2">
                ...
                <div>
                  <strong>Studentas:</strong> {lesson.student}
                </div>
                <div>
                  <strong>Data:</strong> {new Date(lesson.date).toLocaleDateString("lt-LT")}
                </div>
              </li>
            ))}
          </ul>
