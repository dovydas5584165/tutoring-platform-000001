...
export default function PridetiPamoka() {
  ...
  const id = params?.id || "defaultId"; // fallback or error handling
    ...
    <div className="min-h-screen bg-gray-50 text-gray-900 font-sans flex flex-col">
      <header className="bg-white shadow-sm border-b border-gray-200 px-6 py-4 flex justify-between items-center">
        ...
        <nav className="space-x-4 text-sm sm:text-base flex items-center">
          <a href={`/tutor_dashboard/${id}`} className="font-semibold hover:underline">
            Atgal į valdymo skydelį
          </a>
          <button
            type="button"
            onClick={() => router.push(`/pazymiai/${id}`)} // example navigation to grades for this tutor/session
