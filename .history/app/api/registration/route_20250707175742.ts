...
const payloadSchema = z.object({
  ...
  vaikovardas: z.string().optional(), // Note: lowercase 'v' to match the function parameter
...
export async function POST(req: Request) {
  try {
    ...
    console.log("Registration data:", {
      ...
      pavarde,
      role,
      vaikoVardas: vaikovardas, // Log with the name you're using in your code
      pamokos,
    });

    // Use the database function to register the user with exact parameter names
    const { data, error } = await supabaseAdmin.rpc("register_lt_user", {
      ...
      role,
      slaptazodis,
      vaikovardas, // Note: lowercase 'v' to match the function parameter
      vardas,
    });
