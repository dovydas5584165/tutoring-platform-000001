...
>
  {[
    ...
    <motion.div
      key={i}
      className={`relative p-6 rounded-xl shadow-md border-l-4 transition-transform duration-300 hover:scale-105 overflow-hidden ${
    ...
    >
      ...
      <div className="absolute top-0 left-0 w-full h-10 bg-red-600 flex items-center justify-center">
        <span className="text-white font-bold uppercase text-sm">Nauja</span>
      </div>

      <p className="text-gray-800 text-lg mt-4">{tip}</p>
    </motion.div>
  ))}
</motion.div>
