...
export async function POST(req: Request) {
  try {
    ...
    // 2️⃣ Create Auth user via Admin SDK - fix typo here
    ...
    if (signUpErr || !authData?.user) {
      ...
      if (signUpErr?.details) console.error("Details:", signUpErr.details);
      if (signUpErr?.hint) console.error("Hint:", signUpErr.hint);
      return NextResponse.json(
