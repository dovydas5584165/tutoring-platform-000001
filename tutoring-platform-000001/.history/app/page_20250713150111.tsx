...
export default function Home() {
  ...
  /** Map each lesson to a clean URL slug */
  const lessons = [
    { name: "IT", slug: "it" },
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-6">
          {lessons.map(({ name, slug }) => (
            <button
              ...
              className="px-8 py-4 bg-blue-600 rounded-xl hover:bg-blue-700 active:bg-blue-800 focus:bg-blue-800 text-white font-bold text-lg"
            >
              {name}
            </button>
          ))}
