...
type Availability = {
  ...
  start_time: string;
  is_booked: boolean;
  booked_by?: string | null; // who booked this slot
};
...
export default function ScheduleLanding() {
  ...
  useEffect(() => {
    supabase.auth.getUser().then(({ data: { user } }) => {
      if (user) {
        ...
        alert("Prašome prisijungti, kad užsakytumėte pamokas.");
        router.push("/login");
      }
    });
  }, [router]);
  ...
  useEffect(() => {
    const fetchAvailability = async () => {
      ...
      const { data, error } = await supabase
        .from("availability")
        .select("id, user_id, start_time, is_booked, booked_by")
  ...
  const handleBookingConfirmAll = async () => {
    ...
    if (!userId) {
      alert("Prašome prisijungti prieš užsakant pamokas.");
      router.push("/login");
      return;
    }
