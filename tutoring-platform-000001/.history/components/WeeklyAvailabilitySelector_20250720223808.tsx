...
export default function WeeklyAvailabilitySelector() {
  ...
  // Load availability from Supabase for the current user
  useEffect(() => {
  ...
  // Save selected slots to Supabase (replace old with new)
          ...
          <tbody>
            {hours.map((hour) => (
              <tr key={hour} className="text-center">
                ...
                {days.map((_, dayIndex) => {
                  const selected = selectedSlots.some(s => s.day === dayIndex && s.hour === hour);
                  return (
                    <td
                      key={dayIndex}
